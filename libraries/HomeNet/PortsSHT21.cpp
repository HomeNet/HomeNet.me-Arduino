...
void PortsSHT21::clock(uint8_t x) const {
    delayMicroseconds(2);
    digiWrite2(x);
    delayMicroseconds(5);
}

void PortsSHT21::release() const { 
    mode(INPUT);
    digiWrite(1);
}

uint8_t PortsSHT21::writeByte(uint8_t value) const {
    mode(OUTPUT);
...
uint8_t PortsSHT21::readByte(uint8_t ack) const {
    uint8_t value = 0;
    for (uint8_t i = 0x80; i != 0; i >>= 1) {
        clock(1);
        if (digiRead())
...
void PortsSHT21::connReset() const {
    mode2(OUTPUT);
    clock(0);
    mode(OUTPUT);
    digiWrite(1);
...
uint8_t PortsSHT21::measure(uint8_t type, void (*delayFun)()) {
    start();
...
void PortsSHT21::calculate(float& rh_true, float& t_C) const {
  
	
    
    t_C = (((175.72/65536.0) * (float)meas[TEMP]) - 46.85);
	// rh_true = -6.0 + 125.0 / 4096.0;

      rh_true =   -6.0 + 125.0/65536.0  * meas[HUMI];
